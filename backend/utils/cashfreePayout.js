...
const addBeneficiary = async (userId, bankDetails) => {
    ...
    // Unique ID for beneficiary
    const beneficiaryId = `SPL_BENE_${userId}`;
...
const createCashfreePayout = async (user, bankDetails, amount) => {
    ...
    try {
        ...
        const payload = {
            ...
            remarks: `Withdrawal ID ${transferId} for User ${user.userId}`
        };
        ...
        if (response.data.status === 'SUCCESS') {
            ...
            console.log(`✅ Payout created: ${transferId} | Status: ${data.status}`);
            return {
                id: transferId,
                referenceId: data.referenceId || transferId,
                status: data.status || 'SUCCESS'
            };
        } else {
            throw new Error(response.data.message || 'Payout request failed');
        }
    } catch (error) {
